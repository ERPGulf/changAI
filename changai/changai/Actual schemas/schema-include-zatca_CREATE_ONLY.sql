...
-- Description: Stores team member details displayed on the website's About Us page. 
--              Used to manage profile name, image, and bio for public display.
-- Synonyms: Website Team Member, About Page Member, Team Profile, Site Team Member
-- Relationships:
--   - Child link via `parent`, `parenttype`, `parentfield` (part of Website About section or similar parent)
--   - No direct foreign keys, but can be embedded in website sections.

CREATE TABLE `tabAbout Us Team Member` (
  `name` varchar(140) NOT NULL,                     -- Unique document ID (primary key)
  `creation` datetime(6) DEFAULT NULL,              -- Record creation timestamp
  `modified` datetime(6) DEFAULT NULL,              -- Last modification timestamp
  `modified_by` varchar(140) DEFAULT NULL,          -- Last modified by (user)
  `owner` varchar(140) DEFAULT NULL,                -- Record creator/owner
  `docstatus` int(1) NOT NULL DEFAULT 0,            -- Document status (0=Draft, 1=Submitted, 2=Cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                  -- Row index (mainly for child ordering)
  `full_name` varchar(140) DEFAULT NULL,            -- Full name of the team member
  `image_link` text DEFAULT NULL,                   -- URL or path to profile image
  `bio` text DEFAULT NULL,                          -- Short biography/description
  `parent` varchar(140) DEFAULT NULL,               -- Parent document name (for child table)
  `parentfield` varchar(140) DEFAULT NULL,          -- Parent field that links this child table
  `parenttype` varchar(140) DEFAULT NULL,           -- Parent doctype name
  PRIMARY KEY (`name`),
  KEY `parent` (`parent`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;

-- Doctype: Access Log
-- Description: Immutable audit log of user access/export/report actions, capturing who did what, when, and where (document/report/page context).
-- Synonyms: Audit Access Log, Access Audit Entry, Export/Report Access Log
-- Relationships:
--   - Linked to User via `user`
--   - References a doctype via `reference_document` (soft link; no FK)

CREATE TABLE `tabAccess Log` (
  `name`                varchar(140) NOT NULL,        -- Unique document ID (primary key)
  `creation`            datetime(6) DEFAULT NULL,     -- Row creation timestamp
  `modified`            datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`         varchar(140) DEFAULT NULL,    -- User who last modified the row
  `owner`               varchar(140) DEFAULT NULL,    -- Row owner / creator
  `docstatus`           int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                 int(8) NOT NULL DEFAULT 0,    -- Sort index (mainly for child ordering)
  `export_from`         varchar(140) DEFAULT NULL,    -- Source context (e.g., report/export origin)
  `user`                varchar(140) DEFAULT NULL,    -- User who performed the action
  `reference_document`  varchar(140) DEFAULT NULL,    -- Doctype or document referenced during access
  `timestamp`           datetime(6) DEFAULT NULL,     -- When the action occurred
  `file_type`           varchar(140) DEFAULT NULL,    -- Exported file type (e.g., CSV, XLSX, PDF)
  `method`              varchar(140) DEFAULT NULL,    -- Action/method (e.g., view, export, print)
  `report_name`         varchar(140) DEFAULT NULL,    -- Report name (if action relates to a report)
  `filters`             longtext DEFAULT NULL,        -- Serialized filters used for the action
  `page`                longtext DEFAULT NULL,        -- Page/route context (if applicable)
  `columns`             longtext DEFAULT NULL,        -- Selected columns (if applicable)
  `_user_tags`          text DEFAULT NULL,            -- System: user tags
  `_comments`           text DEFAULT NULL,            -- System: comments JSON
  `_assign`             text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`           text DEFAULT NULL,            -- System: liked by JSON
  `_seen`               text DEFAULT NULL,            -- System: seen by JSON
  PRIMARY KEY (`name`),
  KEY `modified` (`modified`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;

-- Doctype: Account
-- Description: Chart of Accounts node/leaf defining GL posting buckets with currency, type, and tree structure (lft/rgt). Supports groups and leaf accounts.
-- Synonyms: COA Account, Chart of Accounts Account, Ledger Account (COA)
-- Relationships:
--   - Linked to Company via `company`
--   - Parent/child account hierarchy via `parent_account` (soft tree with `lft`/`rgt`)
--   - Account type & report classification via `account_type`, `root_type`, `report_type`

CREATE TABLE `tabAccount` (
  `name`                varchar(140) NOT NULL,        -- Unique account ID (primary key)
  `creation`            datetime(6) DEFAULT NULL,     -- Row creation timestamp
  `modified`            datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`         varchar(140) DEFAULT NULL,    -- User who last modified the row
  `owner`               varchar(140) DEFAULT NULL,    -- Row owner / creator
  `docstatus`           int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                 int(8) NOT NULL DEFAULT 0,    -- Sort index
  `disabled`            int(1) NOT NULL DEFAULT 0,    -- If 1, the account is disabled (no postings)
  `account_name`        varchar(140) DEFAULT NULL,    -- Human-readable account name
  `account_number`      varchar(140) DEFAULT NULL,    -- External/ERP account number/code
  `is_group`            int(1) NOT NULL DEFAULT 0,    -- 1=group node (no direct postings), 0=leaf
  `company`             varchar(140) DEFAULT NULL,    -- Company/legal entity this account belongs to
  `root_type`           varchar(140) DEFAULT NULL,    -- Balance Sheet root (Asset/Liability/Equity) or P&L root (Income/Expense)
  `report_type`         varchar(140) DEFAULT NULL,    -- Report classification (Balance Sheet / Profit and Loss)
  `account_currency`    varchar(140) DEFAULT NULL,    -- Currency of this account (override company currency)
  `parent_account`      varchar(140) DEFAULT NULL,    -- Parent account (for hierarchy)
  `account_type`        varchar(140) DEFAULT NULL,    -- Functional type (e.g., Receivable, Payable, Bank, Tax)
  `tax_rate`            decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Default tax rate for this account (if applicable)
  `freeze_account`      varchar(140) DEFAULT NULL,    -- Freeze control (e.g., 'Yes' prevents postings)
  `balance_must_be`     varchar(140) DEFAULT NULL,    -- Constraint: 'Debit' or 'Credit' balance rule
  `lft`                 int(11) NOT NULL DEFAULT 0,   -- Tree left boundary (nested set)
  `rgt`                 int(11) NOT NULL DEFAULT 0,   -- Tree right boundary (nested set)
  `old_parent`          varchar(140) DEFAULT NULL,    -- Previous parent (audit/migration aid)
  `include_in_gross`    int(1) NOT NULL DEFAULT 0,    -- Include in gross profit/loss calculation
  `_user_tags`          text DEFAULT NULL,            -- System: user tags
  `_comments`           text DEFAULT NULL,            -- System: comments JSON
  `_assign`             text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`           text DEFAULT NULL,            -- System: liked by JSON
...
-- Description: Stores the computed closing debit/credit balances for a specific GL account on a given closing date,
--              optionally scoped by company, project, cost center, currency, finance book, and tied to a Period Closing Voucher.
-- Synonyms: Account Closing Balance (GL), Period Closing Balance, GL Closing Snapshot
-- Relationships:
--   - Linked to Account via `account`
--   - Linked to Company via `company`
--   - Linked to Cost Center via `cost_center`
--   - Linked to Project via `project`
--   - Linked to Finance Book via `finance_book`
--   - Linked to Period Closing Voucher via `period_closing_voucher`

CREATE TABLE `tabAccount Closing Balance` (
  `name`                          varchar(140) NOT NULL,        -- Unique row ID (primary key)
  `creation`                      datetime(6) DEFAULT NULL,     -- Row creation timestamp
  `modified`                      datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`                   varchar(140) DEFAULT NULL,    -- User who last modified this row
  `owner`                         varchar(140) DEFAULT NULL,    -- Row owner / creator
  `docstatus`                     int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                           int(8) NOT NULL DEFAULT 0,    -- Sort index
  `closing_date`                  date DEFAULT NULL,             -- Date for which balances are computed
  `account`                       varchar(140) DEFAULT NULL,    -- GL Account
  `cost_center`                   varchar(140) DEFAULT NULL,    -- Cost Center dimension
  `debit`                         decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Closing debit (company currency)
  `credit`                        decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Closing credit (company currency)
  `account_currency`              varchar(140) DEFAULT NULL,    -- Account’s currency
  `debit_in_account_currency`     decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Closing debit in account currency
  `credit_in_account_currency`    decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Closing credit in account currency
  `project`                       varchar(140) DEFAULT NULL,    -- Project dimension
  `company`                       varchar(140) DEFAULT NULL,    -- Company/legal entity
  `finance_book`                  varchar(140) DEFAULT NULL,    -- Finance Book (accounting policy set)
  `period_closing_voucher`        varchar(140) DEFAULT NULL,    -- Related Period Closing Voucher name
  `is_period_closing_voucher_entry` int(1) NOT NULL DEFAULT 0,  -- 1 if generated by a Period Closing Voucher
  `_user_tags`                    text DEFAULT NULL,            -- System: user tags
  `_comments`                     text DEFAULT NULL,            -- System: comments JSON
  `_assign`                       text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`                     text DEFAULT NULL,            -- System: liked by JSON
...
-- Description: Configuration that designates a field on a document type as an accounting dimension (e.g., Project, Cost Center),
--              enabling reporting/filters and propagation into GL/ledger postings.
-- Synonyms: Accounting Dimension Config, Dimension Mapping (Accounting), GL Dimension Definition
-- Relationships:
--   - Targets a document type via `document_type` where the dimension field exists

CREATE TABLE `tabAccounting Dimension` (
  `name`            varchar(140) NOT NULL,        -- Unique configuration ID (primary key)
  `creation`        datetime(6) DEFAULT NULL,     -- Row creation timestamp
  `modified`        datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`     varchar(140) DEFAULT NULL,    -- User who last modified this row
  `owner`           varchar(140) DEFAULT NULL,    -- Row owner / creator
  `docstatus`       int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`             int(8) NOT NULL DEFAULT 0,    -- Sort index
  `document_type`   varchar(140) DEFAULT NULL,    -- Target doctype that holds this dimension field
  `label`           varchar(140) DEFAULT NULL,    -- Display label for the dimension
  `fieldname`       varchar(140) DEFAULT NULL,    -- Fieldname on the target doctype used as the dimension
  `disabled`        int(1) NOT NULL DEFAULT 0,    -- If 1, configuration is inactive
  `_user_tags`      text DEFAULT NULL,            -- System: user tags
  `_comments`       text DEFAULT NULL,            -- System: comments JSON
  `_assign`         text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`       text DEFAULT NULL,            -- System: liked by JSON
...
-- Description: Defines dimension configuration details at the company level, controlling mandatory usage rules 
--              for accounting dimensions (e.g., Cost Center, Project) in Balance Sheet and P&L transactions.
--              It also supports auto-balancing entries using offsetting accounts when required.
-- Synonyms: Accounting Dimension Detail, Dimension Rule Detail, Dimension Control Entry
-- Relationships:
--   - Linked to Company via `company`
--   - References target document via `reference_document`
--   - Parent relationship to `tabAccounting Dimension` via `parent`, `parenttype`, `parentfield`
--   - Optionally links to Account via `offsetting_account`
...
CREATE TABLE `tabAccounting Dimension Detail` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,                            -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                                  -- Sort index
  `company` varchar(140) DEFAULT NULL,                              -- Company/legal entity for which the rule applies
  `reference_document` varchar(140) DEFAULT NULL,                   -- Target doctype to which this dimension rule applies
  `default_dimension` varchar(140) DEFAULT NULL,                    -- Default dimension value (e.g., Cost Center code)
  `mandatory_for_bs` int(1) NOT NULL DEFAULT 0,                      -- If 1, dimension is mandatory for Balance Sheet accounts
  `mandatory_for_pl` int(1) NOT NULL DEFAULT 0,                      -- If 1, dimension is mandatory for Profit & Loss accounts
  `automatically_post_balancing_accounting_entry` int(1) NOT NULL DEFAULT 0, -- If 1, auto-balancing entry is posted when missing
  `offsetting_account` varchar(140) DEFAULT NULL,                   -- Default offsetting account used for auto-balancing
  `parent` varchar(140) DEFAULT NULL,                               -- Parent document name (for child relationship)
  `parentfield` varchar(140) DEFAULT NULL,                          -- Parent field linking this child table
  `parenttype` varchar(140) DEFAULT NULL,                           -- Parent doctype name
  PRIMARY KEY (`name`),
  KEY `parent` (`parent`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;
...
-- Description: Stores company-specific filtering and restriction rules for accounting dimension values 
--              (e.g., restricting certain cost centers or projects for specific scenarios).
--              Used to enforce dimension usage rules at transaction level.
-- Synonyms: Accounting Dimension Filter, Dimension Restriction Rule, Dimension Value Filter
-- Relationships:
--   - Linked to Accounting Dimension via `accounting_dimension`
--   - Linked to Company via `company`
...
CREATE TABLE `tabAccounting Dimension Filter` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,                 -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                       -- Sort index
  `accounting_dimension` varchar(140) DEFAULT NULL,      -- Dimension (e.g., Project, Cost Center) this filter applies to
  `disabled` int(1) NOT NULL DEFAULT 0,                  -- 1 = filter inactive
  `company` varchar(140) DEFAULT NULL,                   -- Company/legal entity
  `apply_restriction_on_values` int(1) NOT NULL DEFAULT 1,  -- 1 = enable value restriction enforcement
  `allow_or_restrict` varchar(140) DEFAULT NULL,         -- Mode: allow or restrict selected values
  `_user_tags` text DEFAULT NULL,                        -- System: user tags
  `_comments` text DEFAULT NULL,                         -- System: comments JSON
  `_assign` text DEFAULT NULL,                           -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,                         -- System: liked by JSON
...
-- Description: Defines a specific fiscal or accounting period for a company. 
--              It is typically used to manage reporting, period closings, tax filings, and other financial operations.
--              Each record represents a named period with defined start and end dates.
-- Synonyms: Accounting Period, Fiscal Period, Financial Period
-- Relationships:
--   - Linked to Company via `company`
--   - Used by Period Closing Voucher and financial reports to determine valid posting windows.
...
CREATE TABLE `tabAccounting Period` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,       -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,            -- Sort index
  `period_name` varchar(140) DEFAULT NULL,    -- Human-readable name of the accounting period (e.g., "Q1 2025")
  `start_date` date DEFAULT NULL,            -- Period start date
  `end_date` date DEFAULT NULL,              -- Period end date
  `company` varchar(140) DEFAULT NULL,        -- Company/legal entity the period applies to
  `_user_tags` text DEFAULT NULL,            -- System: user tags
  `_comments` text DEFAULT NULL,             -- System: comments JSON
  `_assign` text DEFAULT NULL,               -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,             -- System: liked by JSON
...
-- Description: Stores standard billing and costing rates for activities performed by employees. 
--              Used in time tracking, project billing, and cost allocation. 
--              Each entry typically represents an activity type and its corresponding cost structure for an employee.
-- Synonyms: Activity Cost, Employee Activity Cost Rate, Labor Cost Rate
-- Relationships:
--   - Linked to Activity Type via `activity_type`
--   - Linked to Employee via `employee`
--   - Linked to Department via `department`
--   - Used in timesheets, project costing, and billing calculations.
...
CREATE TABLE `tabActivity Cost` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,           -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                 -- Sort index
  `activity_type` varchar(140) DEFAULT NULL,       -- Activity type (e.g., Development, Testing, Design)
  `employee` varchar(140) DEFAULT NULL,            -- Linked employee ID
  `employee_name` varchar(140) DEFAULT NULL,       -- Employee full name
  `department` varchar(140) DEFAULT NULL,          -- Department to which employee belongs
  `billing_rate` decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Rate charged to clients/customers per hour/unit
  `costing_rate` decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Internal cost rate per hour/unit
  `title` varchar(140) DEFAULT NULL,               -- Optional title or description of the activity cost record
  `_user_tags` text DEFAULT NULL,                  -- System: user tags
  `_comments` text DEFAULT NULL,                   -- System: comments JSON
  `_assign` text DEFAULT NULL,                     -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,                   -- System: liked by JSON
...
-- Description: Generic timeline/audit trail entry capturing user/system events (subject, content, operation, status)
--              against a referenced document; also supports timeline/link targets for cross-referencing.
-- Synonyms: Activity Log (Timeline), Timeline Activity Entry, Audit Trail Entry
-- Relationships:
--   - References a primary document via (`reference_doctype`, `reference_name`)
--   - Optional timeline target via (`timeline_doctype`, `timeline_name`)
--   - Optional linked document via (`link_doctype`, `link_name`)
--   - Linked to User via `user`
...
CREATE TABLE `tabActivity Log` (
  ...
  `docstatus`              int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                    int(8) NOT NULL DEFAULT 0,    -- Sort index
  `subject`                text DEFAULT NULL,            -- Short subject/summary of the activity
  `content`                longtext DEFAULT NULL,        -- Detailed content/body (e.g., message, description)
  `communication_date`     datetime(6) DEFAULT NULL,     -- When the activity/communication occurred
  `ip_address`             varchar(140) DEFAULT NULL,    -- IP address (if captured)
  `operation`              varchar(140) DEFAULT NULL,    -- Operation type (e.g., Create, Update, Delete, Email)
  `status`                 varchar(140) DEFAULT NULL,    -- Status/outcome (e.g., Sent, Read, Failed, Success)
  `reference_doctype`      varchar(140) DEFAULT NULL,    -- Primary referenced doctype
  `reference_name`         varchar(140) DEFAULT NULL,    -- Primary referenced document name/ID
  `reference_owner`        varchar(140) DEFAULT NULL,    -- Owner of the referenced document
  `timeline_doctype`       varchar(140) DEFAULT NULL,    -- Secondary timeline doctype (for threaded timeline)
  `timeline_name`          varchar(140) DEFAULT NULL,    -- Secondary timeline doc name/ID
  `link_doctype`           varchar(140) DEFAULT NULL,    -- Additional linked doctype (cross-reference)
  `link_name`              varchar(140) DEFAULT NULL,    -- Additional linked document name/ID
  `user`                   varchar(140) DEFAULT NULL,    -- User responsible for the activity
  `full_name`              varchar(140) DEFAULT NULL,    -- Display name of the user
  `_user_tags`             text DEFAULT NULL,            -- System: user tags
  `_comments`              text DEFAULT NULL,            -- System: comments JSON
  `_assign`                text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`              text DEFAULT NULL,            -- System: liked by JSON
...
-- Description: Master list of activity categories (e.g., Development, Testing, Design) used for time tracking, 
--              costing, and billing in projects. Defines standard billing and costing rates at the activity level.
-- Synonyms: Activity Type, Work Activity Type, Task Activity Category
-- Relationships:
--   - Referenced by Activity Cost (`activity_type`) for employee-specific rates
--   - Referenced in Timesheets, Project Billing, and related costing modules
...
CREATE TABLE `tabActivity Type` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,           -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                 -- Sort index
  `activity_type` varchar(140) DEFAULT NULL,       -- Activity name/category (e.g., Development, Testing)
  `costing_rate` decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Internal cost rate for this activity (per hour/unit)
  `billing_rate` decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Standard billing rate for this activity (per hour/unit)
  `disabled` int(1) NOT NULL DEFAULT 0,            -- 1 = Activity Type disabled
  `_user_tags` text DEFAULT NULL,                  -- System: user tags
  `_comments` text DEFAULT NULL,                   -- System: comments JSON
  `_assign` text DEFAULT NULL,                     -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,                   -- System: liked by JSON
...
-- Doctype: Address
-- Description: Stores structured postal and contact information for customers, suppliers, companies, and other parties. 
--              Used for billing, shipping, taxation, and communication purposes. 
--              An address record can be linked as primary, shipping, or company address.
-- Synonyms: Address, Location Address, Mailing Address, Shipping Address, Billing Address
-- Relationships:
--   - Linked to parties (e.g., Customer, Supplier, Company) as billing/shipping address
--   - Used in Sales, Purchase, and Tax calculation flows via `tax_category`
--   - May be referenced in transactions, e.g., Sales Invoice, Delivery Note, Purchase Order.
...
CREATE TABLE `tabAddress` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,             -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,                   -- Sort index
  `address_title` varchar(140) DEFAULT NULL,         -- Display title for the address (e.g., “Main Office”)
  `address_type` varchar(140) DEFAULT NULL,          -- Type of address (e.g., Billing, Shipping, Office, Home)
  `address_line1` varchar(240) DEFAULT NULL,         -- First line of the street address
  `address_line2` varchar(240) DEFAULT NULL,         -- Second line of the street address (optional)
  `city` varchar(140) DEFAULT NULL,                  -- City
  `county` varchar(140) DEFAULT NULL,                -- County or region
  `state` varchar(140) DEFAULT NULL,                 -- State or province
  `country` varchar(140) DEFAULT NULL,               -- Country
  `pincode` varchar(140) DEFAULT NULL,               -- Postal/ZIP code
  `email_id` varchar(140) DEFAULT NULL,              -- Email contact for this address
  `phone` varchar(140) DEFAULT NULL,                 -- Phone number
  `fax` varchar(140) DEFAULT NULL,                   -- Fax number (if applicable)
  `is_primary_address` int(1) NOT NULL DEFAULT 0,    -- 1 = primary address of linked party
  `is_shipping_address` int(1) NOT NULL DEFAULT 0,   -- 1 = shipping address of linked party
  `disabled` int(1) NOT NULL DEFAULT 0,              -- 1 = inactive address
  `_user_tags` text DEFAULT NULL,                    -- System: user tags
  `_comments` text DEFAULT NULL,                     -- System: comments JSON
  `_assign` text DEFAULT NULL,                       -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,                     -- System: liked by JSON
  `tax_category` varchar(140) DEFAULT NULL,          -- Tax category applicable for this address
  `is_your_company_address` int(1) NOT NULL DEFAULT 0, -- 1 = address belongs to your company entity
  `custom_building_number` varchar(4) DEFAULT NULL,  -- Optional building number (ZATCA/localized field)
  PRIMARY KEY (`name`),
  KEY `city` (`city`),
  KEY `country` (`country`),
  KEY `pincode` (`pincode`),
...
-- Description: Stores country-specific address formatting templates. 
--              Used to generate standardized address displays for invoices, delivery notes, and other documents.
--              Templates help ensure proper layout and localization of address fields per country requirements.
-- Synonyms: Address Template, Address Format Template, Country Address Format, Postal Format Template
-- Relationships:
--   - Linked to Address via `country` field for rendering formatted address
--   - Used in printing and rendering of Sales Invoice, Delivery Note, Purchase Order, etc.
--   - Typically one default template per country (`is_default` flag)
...
CREATE TABLE `tabAddress Template` (
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx` int(8) NOT NULL DEFAULT 0,               -- Sort index
  `country` varchar(140) DEFAULT NULL,           -- Country this template applies to
  `is_default` int(1) NOT NULL DEFAULT 0,        -- 1 = this template is the default for the country
  `template` longtext DEFAULT NULL,              -- Address format structure (HTML/Jinja template)
  `_user_tags` text DEFAULT NULL,                -- System: user tags
  `_comments` text DEFAULT NULL,                 -- System: comments JSON
  `_assign` text DEFAULT NULL,                   -- System: assignments JSON
  `_liked_by` text DEFAULT NULL,                 -- System: liked by JSON
...
-- Description: Tracks movements/application of advance payments against documents (e.g., SO/PO, Invoices).
--              Captures the company, source voucher, target (against) voucher, amount, currency, and event.
-- Synonyms: Advance Payment Ledger Entry, Advance Application Entry, Advance Allocation Row
-- Relationships:
--   - Linked to Company via `company`
--   - Source document via (`voucher_type`, `voucher_no`)
--   - Target/settled document via (`against_voucher_type`, `against_voucher_no`)
...
CREATE TABLE `tabAdvance Payment Ledger Entry` (
  ...
  `idx`                    int(8) NOT NULL DEFAULT 0,    -- Sort index
  `company`                varchar(140) DEFAULT NULL,    -- Company / legal entity
  `voucher_type`           varchar(140) DEFAULT NULL,    -- Source voucher doctype (e.g., Payment Entry)
  `voucher_no`             varchar(140) DEFAULT NULL,    -- Source voucher ID/name
  `against_voucher_type`   varchar(140) DEFAULT NULL,    -- Target voucher doctype (e.g., Sales Invoice, Purchase Invoice)
  `against_voucher_no`     varchar(140) DEFAULT NULL,    -- Target voucher ID/name settled by the advance
  `amount`                 decimal(21,9) NOT NULL DEFAULT 0.000000000,  -- Amount applied
  `currency`               varchar(140) DEFAULT NULL,    -- Currency of the amount
  `event`                  varchar(140) DEFAULT NULL,    -- Event label (e.g., allocate, unallocate, cancel)
  `_user_tags`             text DEFAULT NULL,            -- System: user tags
  `_comments`              text DEFAULT NULL,            -- System: comments JSON
  `_assign`                text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`              text DEFAULT NULL,            -- System: liked by JSON
...
-- Doctype: Advance Tax
-- Description: Stores advance tax allocations applied against invoices or other reference documents.
--              This is typically used to track tax amounts deducted or adjusted in advance payments
--              before final invoice settlement.
-- Synonyms: Advance Tax, Advance Tax Entry, Advance Tax Allocation, Tax Adjustment Entry
-- Relationships:
--   - Linked to source transaction via (`reference_type`, `reference_name`)
--   - Links to accounting head for tax via `account_head`
--   - Child table of other accounting transactions through `parent`, `parentfield`, `parenttype`
...
CREATE TABLE `tabAdvance Tax` (
  ...
  `docstatus`           int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                 int(8) NOT NULL DEFAULT 0,         -- Sort index
  `reference_type`      varchar(140) DEFAULT NULL,         -- Doctype of the source document (e.g., Payment Entry, Sales Invoice)
  `reference_name`      varchar(140) DEFAULT NULL,         -- ID/name of the source document
  `reference_detail`    varchar(140) DEFAULT NULL,         -- Line item or detail reference within the source document
  `account_head`        varchar(140) DEFAULT NULL,         -- Account head associated with the advance tax
  `allocated_amount`    decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Amount of tax allocated
  `parent`              varchar(140) DEFAULT NULL,         -- Parent document ID (when used as child table)
  `parentfield`         varchar(140) DEFAULT NULL,         -- Parent fieldname
  `parenttype`          varchar(140) DEFAULT NULL,         -- Parent doctype
  PRIMARY KEY (`name`),
  KEY `parent` (`parent`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;
...
-- Description: Stores detailed tax and charge line items associated with advance payments.
--              Used to record and allocate tax components (like VAT, GST) and other charges linked
--              to an advance payment, before final invoice settlement.
-- Synonyms: Advance Taxes and Charges, Advance Tax Line, Advance Tax Detail, Advance Charge Entry
-- Relationships:
--   - Linked to Account via `account_head`
--   - Linked to Cost Center via `cost_center`
--   - Typically a child table of an advance payment transaction via `parent`, `parenttype`, `parentfield`
--   - Can be used for tax reconciliation with invoices and payment entries
...
CREATE TABLE `tabAdvance Taxes and Charges` (
  ...
  `docstatus`              int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                    int(8) NOT NULL DEFAULT 0,         -- Sort index
  `add_deduct_tax`         varchar(140) DEFAULT NULL,         -- Whether the tax/charge is added or deducted
  `charge_type`            varchar(140) DEFAULT NULL,         -- Type of charge (e.g., Actual, On Net Total, On Previous Row)
  `row_id`                 varchar(140) DEFAULT NULL,         -- Row ID reference for calculation
  `account_head`           varchar(140) DEFAULT NULL,         -- Ledger account where the tax/charge is posted
  `description`            text DEFAULT NULL,                 -- Description or label for the tax/charge
  `included_in_paid_amount` int(1) NOT NULL DEFAULT 0,        -- Flag indicating if this tax is included in the paid amount
  `cost_center`            varchar(140) DEFAULT NULL,         -- Cost Center associated with the tax/charge
  `rate`                   decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Tax or charge rate (%)
  `currency`               varchar(140) DEFAULT NULL,         -- Currency in which the tax amount is calculated
  `tax_amount`             decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Total tax/charge amount
  `total`                  decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Total taxable amount considered
  `allocated_amount`       decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Portion of tax allocated to specific references
  `base_tax_amount`        decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Tax amount in base currency
  `base_total`             decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Total taxable amount in base currency
  `parent`                 varchar(140) DEFAULT NULL,         -- Parent document ID (when used as child table)
  `parentfield`            varchar(140) DEFAULT NULL,         -- Parent field name
  `parenttype`             varchar(140) DEFAULT NULL,         -- Parent doctype
  PRIMARY KEY (`name`),
...
-- Description: Stores a list of specific dimension values permitted for a parent accounting record.
--              Used to define which accounting dimension values (e.g., cost centers, projects, departments)
--              are allowed in a particular context (e.g., for a company or transaction type).
-- Synonyms: Allowed Dimension, Permitted Dimension, Dimension Permission, Valid Dimension Mapping
-- Relationships:
--   - Links to Accounting Dimension definition via `accounting_dimension`
--   - Links to the specific allowed dimension value via `dimension_value`
--   - Child table of a parent accounting record through `parent`, `parentfield`, `parenttype`
...
CREATE TABLE `tabAllowed Dimension` (
  ...
  `docstatus`              int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                    int(8) NOT NULL DEFAULT 0,         -- Sort index
  `accounting_dimension`   varchar(140) DEFAULT NULL,         -- Reference to the Accounting Dimension (e.g., Cost Center, Project)
  `dimension_value`        varchar(140) DEFAULT NULL,         -- Specific allowed value for that dimension (e.g., Marketing, R&D)
  `parent`                 varchar(140) DEFAULT NULL,         -- Parent document ID (when used as child table)
  `parentfield`            varchar(140) DEFAULT NULL,         -- Parent field name
  `parenttype`             varchar(140) DEFAULT NULL,         -- Parent doctype
  PRIMARY KEY (`name`),
...
-- Description: Defines which companies or entities a parent record (e.g., customer, supplier, party)
--              is allowed to transact with. Used for restricting or validating business transactions.
-- Synonyms: Allowed To Transact With, Permitted Transaction Partner, Transaction Whitelist
-- Relationships:
--   - Linked to Company via `company`
--   - Acts as a child table of another doctype via `parent`, `parenttype`, `parentfield`

CREATE TABLE `tabAllowed To Transact With` (
  `name`         varchar(140) NOT NULL,             -- Unique record ID (primary key)
  `creation`     datetime(6) DEFAULT NULL,          -- Row creation timestamp
  `modified`     datetime(6) DEFAULT NULL,          -- Last modification timestamp
  `modified_by`  varchar(140) DEFAULT NULL,         -- User who last modified the record
  `owner`        varchar(140) DEFAULT NULL,         -- Creator of the record
  `docstatus`    int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`          int(8) NOT NULL DEFAULT 0,         -- Row index
  `company`      varchar(140) DEFAULT NULL,         -- Linked company allowed for transaction
  `parent`       varchar(140) DEFAULT NULL,         -- Parent document name
  `parentfield`  varchar(140) DEFAULT NULL,         -- Field in parent where this child is defined
  `parenttype`   varchar(140) DEFAULT NULL,         -- Parent doctype name
  PRIMARY KEY (`name`),
  KEY `parent` (`parent`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;
...
-- Description: Stores configuration for how amended documents should be named.
--              This defines rules for naming pattern changes (e.g., suffixes, counters)
--              when a document is amended, ensuring unique naming conventions.
-- Synonyms: Amended Document Naming Settings, Amend Naming Rule, Amendment Naming Configuration
-- Relationships:
--   - References the original document type via `document_type`
--   - Acts as a child table of a parent naming configuration doctype via `parent`, `parenttype`, `parentfield`
...
CREATE TABLE `tabAmended Document Naming Settings` (
  ...
  `docstatus`        int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`              int(8) NOT NULL DEFAULT 0,         -- Row index
  `document_type`    varchar(140) DEFAULT NULL,         -- Doctype for which amendment naming settings are defined
  `action`           varchar(140) DEFAULT 'Amend Counter', -- Naming action (e.g., append counter, reset, etc.)
  `parent`           varchar(140) DEFAULT NULL,         -- Parent document name
  `parentfield`      varchar(140) DEFAULT NULL,         -- Field name in parent where this child is defined
  `parenttype`       varchar(140) DEFAULT NULL,         -- Parent doctype name
  PRIMARY KEY (`name`),
...
-- Description: Defines specific accounts (e.g., expense, income, tax accounts) 
--              on which certain rules, charges, or configurations should apply. 
--              Used primarily in accounting or tax-related doctypes to control applicability.
-- Synonyms: Applicable On Account, Account Applicability, Rule Applicable Account
-- Relationships:
--   - Links to Account via `applicable_on_account`
--   - Acts as a child table of its parent configuration via `parent`, `parenttype`, `parentfield`
...
CREATE TABLE `tabApplicable On Account` (
  ...
  `docstatus`               int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                     int(8) NOT NULL DEFAULT 0,         -- Row index
  `applicable_on_account`   varchar(140) DEFAULT NULL,         -- Linked Account to which the rule applies
  `is_mandatory`            int(1) NOT NULL DEFAULT 0,         -- Flag to indicate if this account is mandatory (1=yes, 0=no)
  `parent`                  varchar(140) DEFAULT NULL,         -- Parent document name
  `parentfield`             varchar(140) DEFAULT NULL,         -- Field in parent document where this child is stored
  `parenttype`              varchar(140) DEFAULT NULL,         -- Parent doctype name
  PRIMARY KEY (`name`),
  KEY `parent` (`parent`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;
...
-- Doctype: Appointment
-- Description: Stores scheduled meetings/consultations with a party at a specific date & time,
--              including contact details and a link to a calendar event if created.
-- Synonyms: Appointment, Meeting Booking, Service Appointment
-- Relationships:
--   - Linked party via `party` (e.g., Customer/Lead/Patient depending on implementation)
--   - Linked staff/user via `appointment_with` (e.g., Employee/User)
--   - Linked calendar entry via `calendar_event`
...
CREATE TABLE `tabAppointment` (
  ...
  `modified`               datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`            varchar(140) DEFAULT NULL,    -- User who last modified the record
  `owner`                  varchar(140) DEFAULT NULL,    -- Creator / owner of the record
  `docstatus`              int(1) NOT NULL DEFAULT 0,    -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`                    int(8) NOT NULL DEFAULT 0,    -- Row order index
  `scheduled_time`         datetime(6) DEFAULT NULL,     -- Appointment start date & time
  `status`                 varchar(140) DEFAULT NULL,    -- State (e.g., Scheduled, Completed, No-Show, Cancelled)
  `customer_name`          varchar(140) DEFAULT NULL,    -- Party display name (if customer-oriented flow)
  `customer_phone_number`  varchar(140) DEFAULT NULL,    -- Contact phone
  `customer_skype`         varchar(140) DEFAULT NULL,    -- Skype/VoIP handle
  `customer_email`         varchar(140) DEFAULT NULL,    -- Contact email
  `customer_details`       longtext DEFAULT NULL,        -- Freeform notes about the party / visit reason
  `appointment_with`       varchar(140) DEFAULT NULL,    -- Staff/User the appointment is with (e.g., Employee/User)
  `party`                  varchar(140) DEFAULT NULL,    -- Linked party identifier (e.g., Customer/Lead/Patient)
  `calendar_event`         varchar(140) DEFAULT NULL,    -- Linked Calendar Event ID (if synchronized)
  `_user_tags`             text DEFAULT NULL,            -- System: user tags
  `_comments`              text DEFAULT NULL,            -- System: comments JSON
  `_assign`                text DEFAULT NULL,            -- System: assignments JSON
  `_liked_by`              text DEFAULT NULL,            -- System: liked by JSON
...
-- Doctype: Appointment Booking Slots
-- Description: Defines available booking slots for appointments based on day of the week 
--              and specific time ranges. Used to control when appointments can be scheduled.
-- Synonyms: Appointment Booking Slots, Booking Slot, Appointment Time Slot
-- Relationships:
--   - Acts as a child table of Appointment Settings (or similar configuration) via `parent`, `parenttype`, `parentfield`
--   - Used in relation to Appointment scheduling to validate available slots.
...
CREATE TABLE `tabAppointment Booking Slots` (
  ...
  `docstatus`     int(1) NOT NULL DEFAULT 0,         -- Document status (0=draft, 1=submitted, 2=cancelled)
  `idx`           int(8) NOT NULL DEFAULT 0,         -- Row index
  `day_of_week`   varchar(140) DEFAULT NULL,         -- Day of the week (e.g., Monday, Tuesday)
  `from_time`     time(6) DEFAULT NULL,              -- Start time of the available slot
  `to_time`       time(6) DEFAULT NULL,              -- End time of the available slot
  `parent`        varchar(140) DEFAULT NULL,         -- Parent document name
  `parentfield`   varchar(140) DEFAULT NULL,         -- Field in parent document where this child is stored
  `parenttype`    varchar(140) DEFAULT NULL,         -- Parent doctype name
  PRIMARY KEY (`name`),
...
-- Doctype: Asset
-- Description: Fixed asset master record. Tracks ownership, acquisition details, cost, location/custody,
--              depreciation policy/status, insurance, and links to purchase documents and ledger postings.
-- Synonyms: Asset Master, Fixed Asset
-- Relationships:
--   - Company via `company`
--   - Item master via `item_code` (asset derived from an item)
--   - Asset Category via `asset_category`
--   - Party links via `supplier`, `customer` (e.g., vendor-supplied, customer-assigned)
--   - Costing dimensions via `cost_center`, `department`, `location`, `custodian`
--   - Purchase documents via `purchase_receipt`/`_item`, `purchase_invoice`/`_item`
--   - Finance Book via `default_finance_book`
--   - Journal Entry (scrap) via `journal_entry_for_scrap`

CREATE TABLE `tabAsset` (
  `name`                               varchar(140) NOT NULL,        -- Unique asset ID (primary key)
  `creation`                           datetime(6) DEFAULT NULL,     -- Row creation timestamp
  `modified`                           datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`                        varchar(140) DEFAULT NULL,    -- User who last modified
  `owner`                              varchar(140) DEFAULT NULL,    -- Record owner/creator
  `docstatus`                          int(1) NOT NULL DEFAULT 0,    -- 0=Draft,1=Submitted,2=Cancelled
  `idx`                                int(8) NOT NULL DEFAULT 0,    -- Row index (system)

  `company`                            varchar(140) DEFAULT NULL,    -- Company/legal entity
  `item_code`                          varchar(140) DEFAULT NULL,    -- Source Item code (if created from Item)
  `item_name`                          varchar(140) DEFAULT NULL,    -- Item name (denormalized)
  `asset_owner`                        varchar(140) DEFAULT NULL,    -- Owner type (e.g., Company/Customer/Supplier)
  `asset_owner_company`                varchar(140) DEFAULT NULL,    -- If owner is a company, its name
  `is_existing_asset`                  int(1) NOT NULL DEFAULT 0,    -- 1 if migrated/opening asset (pre-existing)
  `is_composite_asset`                 int(1) NOT NULL DEFAULT 0,    -- 1 if composed of multiple sub-assets
  `supplier`                           varchar(140) DEFAULT NULL,    -- Supplier/vendor (if applicable)
  `customer`                           varchar(140) DEFAULT NULL,    -- Customer (if customer-owned/assigned)
  `image`                              text DEFAULT NULL,            -- Image URL/path

  `journal_entry_for_scrap`            varchar(140) DEFAULT NULL,    -- Journal Entry used when scrapped
  `naming_series`                      varchar(140) DEFAULT NULL,    -- Naming series pattern
  `asset_name`                         varchar(140) DEFAULT NULL,    -- Human-friendly asset name
  `asset_category`                     varchar(140) DEFAULT NULL,    -- Asset category/class
  `location`                           varchar(140) DEFAULT NULL,    -- Physical location/site
  `split_from`                         varchar(140) DEFAULT NULL,    -- Parent asset if this was split
  `custodian`                          varchar(140) DEFAULT NULL,    -- Custodian/responsible person
  `department`                         varchar(140) DEFAULT NULL,    -- Department using the asset
  `disposal_date`                      date DEFAULT NULL,            -- Disposal date (if disposed)
  `cost_center`                        varchar(140) DEFAULT NULL,    -- Cost center associated with the asset

  `purchase_receipt`                   varchar(140) DEFAULT NULL,    -- Linked Purchase Receipt
  `purchase_receipt_item`              varchar(140) DEFAULT NULL,    -- Line item on Purchase Receipt
  `purchase_invoice`                   varchar(140) DEFAULT NULL,    -- Linked Purchase Invoice
  `purchase_invoice_item`              varchar(140) DEFAULT NULL,    -- Line item on Purchase Invoice
  `purchase_date`                      date DEFAULT NULL,            -- Acquisition/purchase date
  `available_for_use_date`             date DEFAULT NULL,            -- Date asset became available for use

  `gross_purchase_amount`              decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Original purchase price
  `asset_quantity`                     int(11) NOT NULL DEFAULT 1,   -- Quantity represented by this record
  `additional_asset_cost`              decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Capitalized additional costs
  `total_asset_cost`                   decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Gross + additional cost

  `calculate_depreciation`             int(1) NOT NULL DEFAULT 0,    -- 1 if depreciation is enabled
  `opening_accumulated_depreciation`   decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Accum. depn at opening
  `opening_number_of_booked_depreciations` int(11) NOT NULL DEFAULT 0, -- Count of booked depn at opening
  `is_fully_depreciated`               int(1) NOT NULL DEFAULT 0,    -- 1 if fully depreciated
  `depreciation_method`                varchar(140) DEFAULT NULL,    -- e.g., Straight Line, WDV
  `value_after_depreciation`           decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Net book value target
  `total_number_of_depreciations`      int(11) NOT NULL DEFAULT 0,   -- Total planned depreciation periods
  `frequency_of_depreciation`          int(11) NOT NULL DEFAULT 0,   -- Frequency (per year) or months/periods
  `next_depreciation_date`             date DEFAULT NULL,            -- Next scheduled depreciation date

  `policy_number`                      varchar(140) DEFAULT NULL,    -- Insurance policy number
  `insurer`                            varchar(140) DEFAULT NULL,    -- Insurer/provider name
  `insured_value`                      varchar(140) DEFAULT NULL,    -- Insured value (text to allow formats)
  `insurance_start_date`               date DEFAULT NULL,            -- Insurance start date
  `insurance_end_date`                 date DEFAULT NULL,            -- Insurance end date
  `comprehensive_insurance`            varchar(140) DEFAULT NULL,    -- Insurance type/flag (textual)
  `maintenance_required`               int(1) NOT NULL DEFAULT 0,    -- 1 if maintenance plan required

  `status`                             varchar(140) DEFAULT 'Draft', -- Asset status (Draft/Submitted/Active/Disposed)
  `booked_fixed_asset`                 int(1) NOT NULL DEFAULT 0,    -- 1 if capitalized/booked to FA
  `purchase_amount`                    decimal(21,9) NOT NULL DEFAULT 0.000000000, -- Alternate purchase amt field
  `default_finance_book`               varchar(140) DEFAULT NULL,    -- Default Finance Book for depreciation
  `depr_entry_posting_status`          varchar(140) DEFAULT NULL,    -- Status of depreciation JE postings
  `amended_from`                       varchar(140) DEFAULT NULL,    -- Source document if amended
...
-- Description: Logs all actions and events performed on an asset, such as transfers, maintenance,
--              depreciation events, usage, or manual notes. Useful for asset history tracking,
--              audit trail, and operational reporting.
-- Synonyms: Asset Activity Log, Asset Event Log, Asset History, Asset Timeline
-- Relationships:
--   - Links to Asset master via `asset`
--   - Optionally linked to a User via `user` to identify who performed the action
--   - Acts as a chronological log for tracking asset lifecycle changes

CREATE TABLE `tabAsset Activity` (
  `name`          varchar(140) NOT NULL,        -- Unique activity record ID (primary key)
  `creation`      datetime(6) DEFAULT NULL,     -- Record creation timestamp
  `modified`      datetime(6) DEFAULT NULL,     -- Last modification timestamp
  `modified_by`   varchar(140) DEFAULT NULL,    -- User who last modified this log entry
  `owner`         varchar(140) DEFAULT NULL,    -- Creator/owner of the activity record
  `docstatus`     int(1) NOT NULL DEFAULT 0,    -- 0=Draft, 1=Submitted, 2=Cancelled
  `idx`           int(8) NOT NULL DEFAULT 0,    -- Row index (system field)

  `asset`         varchar(140) DEFAULT NULL,    -- Linked Asset ID (foreign key to tabAsset)
  `date`          datetime(6) DEFAULT NULL,     -- Date and time of the activity/event
  `user`          varchar(140) DEFAULT NULL,    -- User who performed the activity
  `subject`       text DEFAULT NULL,            -- Description/summary of the activity
...
CREATE TABLE `tabCompany` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `custom__company_name_in_arabic__` varchar(140) DEFAULT NULL,
  `custom_registration_type` varchar(140) DEFAULT 'CRN',
  `custom_company_registration` varchar(140) DEFAULT NULL,
  `custom_zatca__location_for_csr_configuratoin` varchar(140) DEFAULT NULL,
  `custom_zatca__company_category_for_csr_configuration` varchar(140) DEFAULT NULL,
  `custom_zatca_invoice_enabled` int(1) NOT NULL DEFAULT 0,
  `custom_costcenter` int(1) NOT NULL DEFAULT 0,
  `custom_submit_line_item_discount_to_zatca` int(1) NOT NULL DEFAULT 0,
  `custom_phase_1_or_2` varchar(140) DEFAULT 'Phase-1',
  `custom_send_einvoice_background` int(1) NOT NULL DEFAULT 0,
  `custom_select` varchar(140) DEFAULT NULL,
  `custom_send_invoice_to_zatca` varchar(140) DEFAULT 'Live',
  `custom_submit_or_not` int(1) NOT NULL DEFAULT 0,
  `custom_start_time` time(6) DEFAULT NULL,
  `custom_end_time` time(6) DEFAULT NULL,
  `custom_start_time_session` time(6) DEFAULT NULL,
  `custom_end_time_session` time(6) DEFAULT NULL,
  `custom_csr_config` longtext,
  `custom_csr_data` longtext DEFAULT NULL,
  `custom_attach_xml_with_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_attach_xml_with_qr_code` int(1) NOT NULL DEFAULT 0,
  `custom_attach_qr_code_doctype` int(1) NOT NULL DEFAULT 0,
  `custom_attach_e_invoice_send_status_with_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_pih` text,
  `custom_private_key` text DEFAULT NULL,
  `custom_public_key` text DEFAULT NULL,
  `custom_certificate` longtext DEFAULT NULL,
  `custom_sandbox_url` varchar(140) DEFAULT 'https://gw-fatoora.zatca.gov.sa/e-invoicing/developer-portal/',
  `custom_simulation_url` varchar(140) DEFAULT 'https://gw-fatoora.zatca.gov.sa/e-invoicing/simulation/',
  `custom_production_url` varchar(140) DEFAULT ' https://gw-fatoora.zatca.gov.sa/e-invoicing/core/',
  `custom_otp` varchar(140) DEFAULT NULL,
  `custom_basic_auth_from_csid` longtext DEFAULT NULL,
  `custom_compliance_request_id_` text DEFAULT NULL,
  `custom_validation_type` varchar(140) DEFAULT NULL,
  `custom_sample_invoice_number_to_test` varchar(140) DEFAULT NULL,
  `custom_basic_auth_from_production` longtext DEFAULT NULL,
  PRIMARY KEY (`name`),
...
CREATE TABLE `tabCost Center` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `custom_zatca__registration_type` varchar(140) DEFAULT 'CRN',
  `custom_zatca__registration_number` varchar(140) DEFAULT NULL,
  `custom_zatca_branch_address` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
  KEY `lft` (`lft`),
  KEY `rgt` (`rgt`),
  KEY `modified` (`modified`),
...
CREATE TABLE `tabCustomer` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `zatca_customer_name_in_arabic` varchar(140) DEFAULT NULL,
  `custom_b2c` int(1) NOT NULL DEFAULT 0,
  `custom_buyer_id_type` varchar(140) DEFAULT 'TIN',
  `custom_buyer_id` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
...
CREATE TABLE `tabItem Tax Template` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `custom_exemption_reason_code` varchar(140) DEFAULT NULL,
  `custom_zatca_tax_category` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
  KEY `modified` (`modified`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;
...
CREATE TABLE `tabPOS Invoice` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `ksa_einv_qr` text DEFAULT NULL,
  `custom_ksa_einvoicing_xml` varchar(140) DEFAULT NULL,
  `custom_zatca_tax_category` varchar(140) DEFAULT NULL,
  `custom_exemption_reason_code` varchar(140) DEFAULT NULL,
  `custom_uuid` varchar(140) DEFAULT 'Not submitted',
  `custom_zatca_status` varchar(140) DEFAULT 'Not Submitted',
  `custom_zatca_pos_name` varchar(140) DEFAULT NULL,
  `custom_zatca_full_response` longtext DEFAULT NULL,
  PRIMARY KEY (`name`),
  KEY `customer` (`customer`),
  KEY `posting_date` (`posting_date`),
  KEY `return_against` (`return_against`),
...
CREATE TABLE `tabPurchase Invoice` (
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `custom_zatca_summary_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_3rd_party_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_self_billed_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_import_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_nominal_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_tax_category` varchar(140) DEFAULT 'Standard',
  `custom_exemption_reason_code` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
...
CREATE TABLE `tabSales Invoice` (
  ...
  `custom_uuid` text,
  `custom_zatca_status` varchar(140) DEFAULT 'Not Submitted',
  `custom_zatca_third_party_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_nominal_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_export_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_summary_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_self_billed_invoice` int(1) NOT NULL DEFAULT 0,
  `custom_user_invoice_number` varchar(140) DEFAULT NULL,
  `custom_zatca_tax_category` varchar(140) DEFAULT NULL,
  `custom_exemption_reason_code` varchar(140) DEFAULT NULL,
  `custom_zatca_discount_reason_code` varchar(140) DEFAULT '95',
  `custom_zatca_discount_reason` varchar(140) DEFAULT 'Discount',
  `custom_submit_line_item_discount_to_zatca` int(1) NOT NULL DEFAULT 0,
  `custom_zatca_full_response` longtext DEFAULT NULL,
  `custom_zatca_pos_name` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
...
CREATE TABLE `tabWebsite Theme` (
  ...
  `font_properties` varchar(140) DEFAULT 'wght@300';
...
CREATE TABLE `tabZATCA ERPGulf Success Log` (
  `name` varchar(140) NOT NULL,
  `creation` datetime(6) DEFAULT NULL,
  `modified` datetime(6) DEFAULT NULL,
  `modified_by` varchar(140) DEFAULT NULL,
...
CREATE TABLE `tabZATCA Multiple Setting` (
  `name` varchar(140) NOT NULL,
  `creation` datetime(6) DEFAULT NULL,
  `modified` datetime(6) DEFAULT NULL,
  `modified_by` varchar(140) DEFAULT NULL,
  ...
  `_assign` text DEFAULT NULL,
  `_liked_by` text DEFAULT NULL,
  `custom_linked_doctype` varchar(140) DEFAULT NULL,
  `custom_send_pos_invoices_to_zatca_on_background` int(1) NOT NULL DEFAULT 0,
  `custom_csr_config` longtext DEFAULT NULL,
  `custom_csr_data` longtext DEFAULT NULL,
  `custom_pih` varchar(140) DEFAULT NULL,
  `custom_certficate` longtext DEFAULT NULL,
  `custom_private_key` text DEFAULT NULL,
  `custom_public_key` text DEFAULT NULL,
  `custom_otp` varchar(140) DEFAULT NULL,
  `custom_compliance_request_id_` varchar(140) DEFAULT NULL,
  `custom_basic_auth_from_csid` text DEFAULT NULL,
  `custom_final_auth_csid` longtext DEFAULT NULL,
  PRIMARY KEY (`name`),
  KEY `modified` (`modified`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;

CREATE TABLE `tabZatca POS table` (
  `name` varchar(140) NOT NULL,
  `creation` datetime(6) DEFAULT NULL,
  `modified` datetime(6) DEFAULT NULL,
  `modified_by` varchar(140) DEFAULT NULL,
  ...
  `docstatus` int(1) NOT NULL DEFAULT 0,
  `idx` int(8) NOT NULL DEFAULT 0,
  `zatca_multiple_setting` varchar(140) DEFAULT NULL,
  `parent` varchar(140) DEFAULT NULL,
  `parentfield` varchar(140) DEFAULT NULL,
  `parenttype` varchar(140) DEFAULT NULL,
  PRIMARY KEY (`name`),
