...
def _training_prompt(module_name, module_description, schema_context, count):
    return f"""
...
- This is for testing a trained Model in retrieval, so focus on variety and production oriented questions.
...
# @frappe.whitelist(allow_guest=True)
# def generate_validation_data(module_name: str, total_count: int = TARGET_COUNT):
#     module_examples = {
...
#             module_name,
#             module_examples.get(module_name, "ERPNext business questions"),
#             schema_context,
...
@frappe.whitelist(allow_guest=True)
def generate_validation_data(module_name: str,module_desc: str,total_count: int = TARGET_COUNT):
    """
    Generates VALIDATION data = 25% of total_count
    """

    VALIDATION_RATIO = 0.25
    validation_target = max(1, int(total_count * VALIDATION_RATIO))
    ...
    schema_context = build_schema_context_for_module(module_name, module_desc)
    if not schema_context:
        frappe.throw(f"No schema found for module {module_name}")

    folder = ensure_validation_folder()
    filename = f"{module_name.lower()}_validation.jsonl"

    existing = load_existing_records(folder, filename)
    start_index = len(existing) + 1
    remaining = validation_target - len(existing)
    ...
    if remaining <= 0:
        return {
            ...
            "message": "Validation target already satisfied"
        }
    ...
    while remaining > 0 and attempts < MAX_ATTEMPTS:
        ...
        for obj in data:
            ...
            new_records.append({
                "qid": f"{module_name}_VAL_{str(start_index).zfill(3)}",
                "anchor": obj["anchor"],
                "positives": obj["positives"]
            })
    ...
    return {
        ...
        "attempts": attempts,
        "message": (
            f"Validation data generated: {len(existing) + len(new_records)} / {validation_target}"
        )
    }
#*************************************************************************************************************
@frappe.whitelist()
def generate_validation_data_for_modules(modules, total_count: int = TARGET_COUNT):
    """
    Generate validation data ONLY for selected modules
    """
    ...
    for row in modules:
        ...
        module_des=row.get("module_name")
        ...
        try:
            ...
            frappe.log_error(
                ...
                f"ChangAI validation generation failed for {module_name}"
            )
            results.append({
                "module": module_name,
                "ok": False,
