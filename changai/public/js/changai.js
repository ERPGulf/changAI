console.log("ChangAI chatbot JS loaded v1")
...
// BUG FIX #5: Escape user-supplied text before inserting into innerHTML
...
function initChangAIChatbot() {
  if (document.getElementById("chatbot-toggler")) return;
  ...
  function renderMessages() {
    ...
    if (activeTab === "chat") {
      // BUG FIX #3: Removed double-nested messageCon wrapper. The welcome
      // message now has a single wrapper div with the correct classes.
      const welcome = document.createElement("div");
      welcome.className = "messageCon bot-message";
      welcome.innerHTML = `${BOT_SVG}<p class="message-text">Hello there 👋 I am ChangAI, your ERP assistant</p>`;
      ...
      chatHistory.forEach((msg) => {
        ...
        // BUG FIX #5: Escape text to prevent XSS
      ...
      if (debugLogs.length === 0) {
        ...
        debugLogs.forEach((log) => {
          ...
          pre.textContent = safeStringify(log);
      ...
      if (supportHistory.length === 0) {
        ...
        supportHistory.forEach((msg) => {
          const div = document.createElement("div");
          // BUG FIX #4: Consistent role check — 'user' vs everything else (bot/support agent)
          div.className = `messageCon ${msg.role === "user" ? "user-message" : "bot-message"}`;
          // BUG FIX #5: Escape text to prevent XSS
          const safeText = escapeHTML(msg.text);
  ...
  function sendSupportMessage(message) {
    ...
    renderMessages();
    scrollToBottom();
    const thinkingMsg = { role: "model", text: "Sending to support..." };
    supportHistory.push(thinkingMsg);
    renderMessages();
    scrollToBottom();

    frappe.call({
      method: "changai.changai.api.v2.text2sql_pipeline_v2.support_bot",
      args: { message: message },
      callback: function (r) {
        console.log("Support API Success:", r);
        thinkingMsg.text = r.message
          ? safeStringify(r.message)
  ...
  function setChatHistory(message) {
    ...
    try {
      frappe.call({
        ...
        callback: function (r) {
          ...
          debugLogs.push({ user: userMsg, response: r.message });
          renderMessages();
          scrollToBottom();
        },
        error: function (err) {
      ...
      debugLogs.push({ user: userMsg, error: errorText });
      renderMessages();
      scrollToBottom();
    }
  }
  ...
  // Show chatbot closed on init
  toggleChatbot(false);
}

// BUG FIX #1 & #2: Moved the DOMContentLoaded guard OUTSIDE initChangAIChatbot
// so the function doesn't recursively call itself.
